                ...
                <Grid item xs={12} md={7}>
                            ...
                            return (
                                <Box key={type} onClick={() => setData({ ...data, priceType: type.toLowerCase() })}
                                     sx={{ p: 1.5, flex: 1, textAlign: 'center', cursor: 'pointer', borderRadius: '4px', fontSize: '12px', fontWeight: 'bold',
                                         border: data.priceType === type.toLowerCase() ? `1px solid ${isTypeModified ? '#FFC107' : (editMode ? '#4CAF50' : theme.palette.primary.main)}` : `1px solid ${theme.palette.divider}`,
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={4}>
                            ...
                            <Autocomplete options={CURRENCY_OPTIONS} getOptionLabel={(option) => option.label} value={selectedCurrency} onChange={(event, newValue) => setData({ ...data, currency: newValue ? newValue.code : 'SAR' })} disableClearable renderInput={(params) => <TextField {...params} placeholder="Search..." sx={getFieldStyle('currency', data.currency)} />} />
                        </Grid>
                        <Grid item xs={12} sm={4}>
                            ...
                            {['before_acceptance', 'after_acceptance', 'before_payment'].map((policy) => {
                                ...
                                return (
                                    <FormControlLabel key={policy}
                                                      control={<Checkbox sx={{ color: isModified ? '#FFC107' : (editMode ? '#4CAF50' : theme.palette.primary.main), '&.Mui-checked': { color: isModified ? '#FFC107' : (editMode ? '#4CAF50' : theme.palette.primary.main) } }} checked={!!data[`cancel_${policy}`]} onChange={(e) => setData({ ...data, [`cancel_${policy}`]: e.target.checked })} />}
                                                      label={<Typography sx={{ fontSize: '13px', color: isModified ? '#FFC107' : (editMode ? '#4CAF50' : 'inherit') }}>Cancellation {policy.replace(/_/g, ' ')}</Typography>}
                                                      sx={{ width: '100%', ml: 0 }}
                                    />
                                )})}
                        </Box>
